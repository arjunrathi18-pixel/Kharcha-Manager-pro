...
/* ===================== USER-CREATED (PERMANENT) CATEGORIES ===================== */
// Jab "Others" mein koi naam pehli baar type hota hai, wo yahan save ho jaata hai aur
// uske baad ek normal tile ki tarah dikhta hai — dobara naam type nahi karna padta.
// Firestore se load hokar isme bhara jaata hai (dekho KharchaApp._onAuthChanged).
...
const hiStrings = {
  ...
  '(tap karke entries dekho)': '(\u090f\u0902\u091f\u094d\u0930\u0940\u091c \u0926\u0947\u0916\u0928\u0947 \u0915\u0947 \u0932\u093f\u090f \u091f\u0948\u092a \u0915\u0930\u0947\u0902)',
  'Date': '\u0924\u093e\u0930\u0940\u0916',
  'Amount': '\u0930\u093e\u0936\u093f',
        ...
        MaterialPageRoute(
          builder: (_) {
            return PresenceCalendarScreen(
              title: 'Water Tracker',
              categoryKey: 'water',
              instructionText: 'Jis din PAANI AAYA, us date par tap karo',
              presentLabel: 'Paani aaya',
              absentLabel: 'Paani nahi aaya',
...
class _MilkScreenState extends State<MilkScreen> {
  ...
  Future<void> exportMonthExcel() async {
    ...
    try {
      ...
      sheet.appendRow([
        xls.TextCellValue('Litre/din: ${widget.litresPerDay}'),
      ...
      for (int d = 1; d <= daysInMonth; d++) {
        ...
        sheet.appendRow([
          ...
          xls.TextCellValue(isLeave ? 'Leave' : 'Milk Aaya'),
      ...
      sheet.appendRow([]);
      sheet.appendRow([xls.TextCellValue('Total Din'), xls.IntCellValue(daysInMonth)]);
      sheet.appendRow([xls.TextCellValue('Leaves'), xls.IntCellValue(leaves)]);
      sheet.appendRow([xls.TextCellValue('Milk Delivered Din'), xls.IntCellValue(daysInMonth - leaves)]);
      ...
      if (bytes == null) {
        throw Exception('Excel file generate nahi ho payi');
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Excel export mein error aayi: $e')));
      }
    } finally {
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 4),
                  const Text(
                    'Is mahine ka pura din-wise milk record (delivered/leave, litre, amount) Excel file (.xlsx) mein nikaalo aur share karo.',
                  ...
                  SizedBox(
                    ...
                    child: OutlinedButton.icon(
                      ...
                      label: Text(
                        exporting
                            ? 'Excel Taiyaar Ho Raha Hai...'
                            : '${DateFormat('MMMM yyyy').format(cursor)} Ka Excel Nikaalo',
                      ),
                    ),
                  ),
                ],
...
class _ElectricityScreenState extends State<ElectricityScreen> {
  ...
  void save() {
    ...
    if (curr == null || prev == null || rate == null || curr <= prev) {
      setState(() => err = 'Current reading, previous reading se zyada honi chahiye');
      return;
    }
    final entry = Entry(
      id: uuid.v4(),
              ...
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(
                    width: double.infinity,
                    height: 46,
                    child: ElevatedButton(onPressed: save, child: const Text('Bill Record Karo')),
                  ),
                ],
              ),
            ),
...
class _GasScreenState extends State<GasScreen> {
  ...
  void save() {
    ...
    if (amt == null || kg == null || days <= 0) {
      setState(() => err = 'Naye cylinder ki date, purani date se aage honi chahiye, aur amount/kg bharo');
      return;
    }
    final entry = Entry(
      id: uuid.v4(),
      ...
      quantity: kg,
      unitLabel: 'Kg',
      notes: '$days din chala, ${kgPerDay.toStringAsFixed(3)} kg/din, \u20b9${amountPerDay.toStringAsFixed(2)}/din',
      createdAt: DateTime.now().millisecondsSinceEpoch,
    );
    widget.onRecordPayment(entry);
              ...
              child: Column(
                ...
                children: [
                  ...
                  TextField(
                    ...
                    decoration: const InputDecoration(labelText: 'Pichla Cylinder Kab Aaya Tha', suffixIcon: Icon(Icons.calendar_today)),
                    onTap: () async {
                      final picked = await showDatePicker(
                        context: context,
                  ...
                  TextField(
                    ...
                    decoration: const InputDecoration(labelText: 'Naya Cylinder Kab Aaya', suffixIcon: Icon(Icons.calendar_today)),
                    onTap: () async {
                      final picked = await showDatePicker(
                        context: context,
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  const Text('Calculation', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
                  const Divider(height: 20),
                  Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text('Cylinder kitne din chala'), Text('$days din', style: const TextStyle(fontWeight: FontWeight.bold))]),
                  const SizedBox(height: 6),
                  Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text('Per Day Consumption'), Text('${kgPerDay.toStringAsFixed(3)} kg/din', style: const TextStyle(fontWeight: FontWeight.bold))]),
                  const SizedBox(height: 6),
                  Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text('Per Day Cost'), Text('\u20b9${amountPerDay.toStringAsFixed(2)}/din', style: const TextStyle(fontWeight: FontWeight.bold))]),
                  const SizedBox(height: 10),
                  Container(
                    padding: const EdgeInsets.all(12),
                    decoration: BoxDecoration(color: goldColor.withOpacity(0.15), borderRadius: BorderRadius.circular(10)),
                  ...
                  SizedBox(
                    width: double.infinity,
                    height: 46,
                    child: ElevatedButton(onPressed: save, child: const Text('Cylinder Record Karo')),
                  ),
                ],
              ),
            ),
...
class _RechargeScreenState extends State<RechargeScreen> {
  ...
  void save() {
    ...
    if (validity == null || validity <= 0 || price == null || price <= 0) {
      setState(() => err = 'Validity aur price sahi se bharo');
      return;
    }
    final entry = Entry(
      id: uuid.v4(),
      ...
      quantity: validity,
      unitLabel: 'Din',
      notes: '$validity din ka plan, \u20b9${perDayCost.toStringAsFixed(2)}/din (pichla recharge $daysSincePrev din pehle tha)',
      createdAt: DateTime.now().millisecondsSinceEpoch,
    );
    widget.onRecordPayment(entry);
              ...
              child: Column(
                ...
                children: [
                  ...
                  TextField(
                    readOnly: true,
                    controller: TextEditingController(text: prevDate),
                    decoration: const InputDecoration(labelText: 'Pichla Recharge Kab Kiya Tha', suffixIcon: Icon(Icons.calendar_today)),
                    onTap: () async {
                      final picked = await showDatePicker(
                        context: context,
                        initialDate: DateTime.parse(prevDate),
                  ...
                  TextField(
                    readOnly: true,
                    controller: TextEditingController(text: newDate),
                    decoration: const InputDecoration(labelText: 'Naya Recharge Kab Kiya', suffixIcon: Icon(Icons.calendar_today)),
                    onTap: () async {
                      final picked = await showDatePicker(
                        context: context,
                        initialDate: DateTime.parse(newDate),
                  ...
                  TextField(
                    ...
                    keyboardType: TextInputType.number,
                    decoration: const InputDecoration(labelText: 'Validity (Din)'),
                    onChanged: (_) => setState(() {}),
                  ),
                  const SizedBox(height: 10),
                  TextField(
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  const Text('Calculation', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
                  const Divider(height: 20),
                  Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text('Pichla recharge kitne din pehle'), Text('$daysSincePrev din', style: const TextStyle(fontWeight: FontWeight.bold))]),
                  const SizedBox(height: 6),
                  Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text('Per Day Cost'), Text('\u20b9${perDayCost.toStringAsFixed(2)}/din', style: const TextStyle(fontWeight: FontWeight.bold))]),
                  const SizedBox(height: 10),
                  Container(
                    padding: const EdgeInsets.all(12),
                    decoration: BoxDecoration(color: goldColor.withOpacity(0.15), borderRadius: BorderRadius.circular(10)),
                  ...
                  SizedBox(
                    width: double.infinity,
                    height: 46,
                    child: ElevatedButton(onPressed: save, child: const Text('Recharge Record Karo')),
                  ),
                ],
              ),
            ),
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 4),
                  const Text('Kaunsa plan value for money hai, compare karo', style: TextStyle(color: Colors.grey, fontSize: 12)),
                  const SizedBox(height: 10),
                  Wrap(
                    spacing: 8,
                    runSpacing: 8,
                    children: [28, 56, 84, 180, 365].map((d) {
                      return OutlinedButton(onPressed: () => addPlan(d), child: Text('+ $d din'));
                    }).toList(),
                  ),
                  const SizedBox(height: 6),
                  TextButton.icon(
                    ...
                    label: const Text('Custom Plan Add Karo'),
                  ),
                  const SizedBox(height: 10),
                            ...
                            children: [
                              Expanded(
                                child: TextField(
                                  controller: vCtrl,
                                  keyboardType: TextInputType.number,
                                  decoration: const InputDecoration(labelText: 'Validity (din)', isDense: true),
                                  onChanged: (_) => setState(() {}),
                                ),
                              ),
                              const SizedBox(width: 8),
                            ...
                            Padding(
                              ...
                              child: Row(
                                ...
                                children: [
                                  ...
                                  Text('\u20b9${perDay.toStringAsFixed(2)} / din', style: TextStyle(fontWeight: FontWeight.bold, color: isBest ? goldColor : null)),
                                ],
                              ),
                            ),
                        ],
                  ...
                  if (plans.isEmpty)
                    const Padding(
                      ...
                      child: Text('Upar se plan add karke price compare karo', style: TextStyle(color: Colors.grey, fontSize: 12)),
                    ),
                ],
              ),
            ),
      ...
      final action = await showDialog<String>(
        ...
        builder: (_) {
          return AlertDialog(
            ...
            actions: [
              TextButton(onPressed: () => Navigator.pop(context, 'remove'), child: const Text('Hatao')),
              TextButton(onPressed: () => Navigator.pop(context, 'cancel'), child: const Text('Band Karo')),
              TextButton(onPressed: () => Navigator.pop(context, 'save'), child: const Text('Save')),
            ],
          );
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const Divider(height: 24),
            statRow('Total din', '$totalDays'),
            statRow('${widget.presentLabel} din', '$presentCount'),
            statRow('Rate', '\u20b9${widget.ratePerDay.toStringAsFixed(2)} / din'),
            const Divider(height: 24),
            Container(
              padding: const EdgeInsets.all(12),
              decoration: BoxDecoration(color: goldColor.withOpacity(0.15), borderRadius: BorderRadius.circular(10)),
                    ...
                    child: ElevatedButton(
                      onPressed: () {
                        ...
                        final entry = Entry(
                          ...
                          date: DateFormat('yyyy-MM-dd').format(lastDay),
                          amount: amount,
                          notes: '$presentCount/$totalDays din ${widget.presentLabel} ($rangeLabel)',
                          createdAt: DateTime.now().millisecondsSinceEpoch,
                        );
                        widget.onRecordPayment(entry);
                        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Payment record ho gaya')));
        ...
        children: [
          ...
          if (isCurrentMonth) ...[
            Builder(builder: (_) {
              ...
              final rangeTillToday = '${DateFormat('d MMM').format(firstDay)} - ${DateFormat('d MMM yyyy').format(now)}';
              return calcCard('Abhi Tak Ka Hisaab', rangeTillToday, todayDay, presentTillToday, amountTillToday);
            }),
          ],
          calcCard('Poore Mahine Ka Hisaab', rangeLabel, daysInMonth, presentThisMonth, totalAmount, isFinal: true),
        ],
      ),
    );
  }
...
class _GroceryScreenState extends State<GroceryScreen> {
  ...
  Future<void> addCustomItem() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (_) {
        return AlertDialog(
          title: const Text('Custom Item Add Karo'),
          content: TextField(
            controller: nameCtrl,
            autofocus: true,
            decoration: const InputDecoration(labelText: 'Item ka naam likho'),
          ),
          actions: [
            TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Cancel')),
            TextButton(onPressed: () => Navigator.pop(context, true), child: const Text('Aage Badho')),
          ],
        );
      },
    );
          ...
          Padding(
            ...
            child: SizedBox(
              ...
              child: OutlinedButton.icon(
                ...
                label: const Text('Sirf Total Amount Add Karo (item list ke bina)'),
              ),
            ),
          ),
          Padding(
          ...
          Padding(
            ...
            child: SizedBox(
              ...
              child: OutlinedButton.icon(
                ...
                label: const Text('Custom Item Add Karo (list mein nahi hai)'),
              ),
            ),
          ),
          Expanded(
...
class _GroceryItemSheetState extends State<GroceryItemSheet> {
  ...
  void save() {
    final amt = double.tryParse(amountCtrl.text) ?? 0;
    if (amt <= 0) {
      setState(() => err = 'Valid amount daalo');
      return;
    }
    final entry = Entry(
      id: uuid.v4(),
...
class _AddTabState extends State<AddTab> {
  ...
  Future<void> _confirmDeleteCustomCategory(Category c) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (_) {
        return AlertDialog(
          title: const Text('Category Hatao?'),
          content: Text('"${c.label}" tile hamesha ke liye hat jayegi. Pehle ki entries safe rahengi, sirf tile hatega.'),
          actions: [
            TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Cancel')),
            TextButton(onPressed: () => Navigator.pop(context, true), child: const Text('Hatao')),
          ],
        );
      },
    );
        ...
        children: [
          ...
          if (catsFor(section, vehicleType).any((c) => c.generated))
            const Padding(
              ...
              child: Text(
                'Naye naam wali tiles ko der tak dabao (long-press) unhe hatane ke liye.',
            ...
            children: [
              ...
              if (widget.category.custom) ...[
                ...
                const Padding(
                  ...
                  child: Text(
                    'Ye naam save karte hi permanent tile ban jayega, dobara type nahi karna padega.',
                    style: TextStyle(fontSize: 11, color: Colors.grey),
                  ),
                ),
              ],
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        ...
        const SizedBox(height: 4),
        Text(tr('(tap karke entries dekho)', lang), style: const TextStyle(fontSize: 11, color: Colors.grey)),
        const SizedBox(height: 10),
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 6),
                  const Text(
                    'Abhi Navigation, Section aur Category names Hindi mein badalte hain. Baaki screens jaldi aayengi.',
                    style: TextStyle(fontSize: 11, color: Colors.grey),
                  ),
                ],
              ),
