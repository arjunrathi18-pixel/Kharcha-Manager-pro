...
class _KharchaAppState extends State<KharchaApp> {
  ...
  void _onAuthChanged(User? user) {
    ...
    docSub = docRef.snapshots().listen((snap) async {
      if (!snap.exists) {
        // IMPORTANT: right after a fresh install (empty local Firestore cache),
        // the very first snapshot can come straight from that empty cache and
        // report "doesn't exist" even though the real document is sitting on
        // the server — the client just hasn't heard back yet. If we trusted
        // that and wrote fresh defaults here, we would overwrite the user's
        // real cloud data with a blank profile the moment they log back in.
        // Only create a new document once Firestore has actually confirmed
        // with the server that nothing is there; otherwise wait quietly for
        // the next (server-backed) snapshot.
