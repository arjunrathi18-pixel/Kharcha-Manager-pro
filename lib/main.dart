...
// When a name is typed into "Others" for the first time, it's saved here and
// shown as a normal tile from then on — no need to retype it.
// Loaded from Firestore at runtime (see KharchaApp._onAuthChanged).
...
class _KharchaAppState extends State<KharchaApp> {
  ...
  void _onAuthChanged(User? user) async {
    ...
    // Step 1: find out — reliably and *actively* — whether this user's document
    // already exists, by asking the server directly first. This avoids ever
    // trusting an empty local cache (which is exactly what you get right after
    // a fresh install or update) as proof that there's no data. If the server
    // can't be reached quickly, we fall back to whatever is cached locally
    // instead of just hanging on a passive listener with nothing to show for it.
    try {
      DocumentSnapshot<Map<String, dynamic>> initialSnap;
      try {
        initialSnap = await docRef.get(const GetOptions(source: Source.server));
      } catch (_) {
        initialSnap = await docRef.get(const GetOptions(source: Source.cache));
      }
      ...
      // Both the server and the local cache failed to answer (e.g. genuinely
      // no network at all on a brand-new install). There's nothing more we
      // can actively do here — the snapshots() listener below will still pick
      // things up the moment any connectivity/cached data becomes available.
    }

    // Step 2: subscribe for live updates from here on, exactly as before.
    docSub = docRef.snapshots().listen((snap) {
      if (!snap.exists) {
        // Existence was already resolved (and, if needed, created) above;
        // just wait quietly for the next update.
        return;
      }
