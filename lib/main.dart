...
class _KharchaAppState extends State<KharchaApp> {
  ...
  void toggleMilkLeave(String date, String reason) {
    setState(() {
      if (milkLeaves.containsKey(date)) {
        milkLeaves.remove(date);
      } else {
        milkLeaves[date] = reason;
      }
    });
    _pushMilk();
  }
...
class HomeShell extends StatefulWidget {
  ...
  final void Function(String, String) onToggleMilkLeave;
...
class _HomeShellState extends State<HomeShell> {
  ...
  Widget build(BuildContext context) {
    final tabs = [
      ...
      AddTab(onPick: openEntrySheet, entries: widget.entries),
      CalendarTab(
...
class MilkScreen extends StatefulWidget {
  ...
  final void Function(String date, String reason) onToggleLeave;
...
class _MilkScreenState extends State<MilkScreen> {
  ...
  Future<void> handleDayTap(String iso, bool isFuture) async {
      ...
      builder: (_) {
        return AlertDialog(
          ...
          content: TextField(
            ...
            decoration: const InputDecoration(labelText: 'Reason (optional)', hintText: 'e.g. Milkman nahi aaya'),
            maxLines: 2,
          ),
          actions: [
            TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Cancel')),
    ...
    if (confirmed == true) {
      ...
      widget.onToggleLeave(iso, reasonCtrl.text.trim());
    }
  }
...
class CategoryEntriesScreen extends StatelessWidget {
  ...
  const CategoryEntriesScreen({super.key, required this.title, required this.entries});
            ...
            child: sorted.isEmpty
                ...
                : ListView(
                    children: sorted.map((e) {
                      return EntryTile(entry: e, onDelete: () {});
                    }).toList(),
                  ),
          ),
        ],
...
class AddTab extends StatefulWidget {
  ...
  const AddTab({super.key, required this.onPick, required this.entries});
                      ...
                      child: InkWell(
                        ...
                        onTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (_) => CategoryEntriesScreen(title: c.label, entries: catEntries),
                            ),
                          );
                        },
                        child: Container(
...
class DashboardTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final todayExpense = sum((e) => isExpenseSection(e.section) && e.date == today);
