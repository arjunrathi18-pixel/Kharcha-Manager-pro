...
class _KharchaAppState extends State<KharchaApp> {
  ...
  Future<void> _load() async {
    ...
    if (entriesRaw != null) {
      try {
        final decoded = jsonDecode(entriesRaw) as List;
    ...
    if (settingsRaw != null) {
      try {
        ...
        pin = s['pin'];
        if (s['budgets'] != null) {
          final rawBudgets = s['budgets'] as Map;
  ...
  Future<void> _saveEntries() async {
    ...
    await prefs.setString('km_entries', jsonEncode(list));
  }

  Future<void> _saveReminders() async {
    final prefs = await SharedPreferences.getInstance();
    final list = reminders.map((e) => e.toJson()).toList();
    await prefs.setString('km_reminders', jsonEncode(list));
  }

  Future<void> _saveSettings() async {
    final prefs = await SharedPreferences.getInstance();
    final data = {'dark': dark, 'pin': pin, 'budgets': budgets};
    await prefs.setString('km_settings', jsonEncode(data));
  }
  ...
  Widget build(BuildContext context) {
    ...
    } else if (locked && pin != null) {
      home = LockScreen(pin: pin!, onUnlock: () => setState(() => locked = false));
    } else {
      home = HomeShell(
        entries: entries,
        reminders: reminders,
